    ...
    <div class="modal-backdrop">
      <div class="modal">
        <header class="modal-header">
          <slot name="header">Clone course</slot>
        ...
        <footer class="modal-footer">
          <button type="button" class="btn-green" @click="cloneCourse">
...
export default {
  ...
  methods: {
    ...
    async cloneCourse() {
      ...
      let t = "";
      let values = [];
      let course = this.courses[this.courses.length - 1];
      Object.keys(course).map((key, index) => {
        values.push(course[key]);
      });
      t = values.join("cmsplit");
      console.log(t);
      t =
